...
class Order(models.Model):
    ...
    front_page_body = fields.Html(string="Page de garde", default="<span>Bonjour ${object.partner_id.title.name} ${object.partner_id.title.name}, <br/><br/>Suite à votre demande pour laquelle je vous remercie, veuillez trouver ci-dessous l'offre de prix.<br/><br/>Espérant vous être utile par cette proposition et restant à votre disposition pour tout renseignement complémentaire, je vous prie d'agréer, ${object.partner_id.title.name}, mes salutations les plus distinguées. <br/><br/>${order.user_id.name}<br/>${order.user_id.login}<br/>${order.user_id.phone}<span>")
    front_page_body_template = fields.Html(string="Page de garde template")
