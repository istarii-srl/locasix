        ...
        for wizard in self:
            ...
            for line in wizard.line_ids:
                if line.product_id:
                    ...
                    new_agg_id = self.env["locasix.agg.aller"].search([("date", "=", wizard.aller_date), ("address_id", "=", wizard.order_id.partner_id.id), ("day_id", "=", newday_id.id)], limit=1)
                    if not new_agg_id:
                    ...
                    if wizard.should_create_retour:
                        ...
                        new_agg_id = self.env["locasix.agg.retour"].search([("date", "=", wizard.retour_date), ("address_id", "=", wizard.order_id.partner_id.id), ("day_id", "=", newday_id.id)], limit=1)
                        if not new_agg_id:
                        ...
                        self.env["locasix.retour"].create({
                            "day_id": newday_id.id,
                            "date": new_agg_id.date,
                            "agg_id": new_agg_id.id,
                            "order_id": wizard.order_id,
